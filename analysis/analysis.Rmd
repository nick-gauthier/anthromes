...
contemp_vars <- read_sf('data/contemp_vars.shp') %>%
  ...
  select(-c(max, land_ar, landsut, pop17, food_cl, mean, nmh, L1_ID))
```
...
Create summaries of global land areas for each anthrome along with breakdowns by region and biome. See the documentaiton for the *anthrome_trajectory()* function for more details.
```{r, cache = TRUE}
global_summary <- an12_dgg %>% 
  select(-lower, -upper) %>% 
  rename(anthrome = baseline) %>% 
  anthrome_trajectory() %>%
  # .drop = false perserves annual values!
  filter(!is.na(period))
...
country_summary_lower <- an12_dgg %>% 
  select(-baseline, -upper) %>% 
  rename(anthrome = lower) %>% 
  anthrome_trajectory(by = country_name)%>%
  filter(!is.na(period))

country_summary_upper <- an12_dgg %>% 
  select(-baseline, -lower) %>% 
  rename(anthrome = upper) %>% 
  anthrome_trajectory(by = country_name)%>%
  filter(!is.na(period))

country_summary <- an12_dgg %>% 
  select(-lower, -upper) %>% 
  rename(anthrome = baseline) %>% 
  anthrome_trajectory(by = country_name) %>%
  filter(!is.na(period))
...
an12_dgg %>% 
  ...
  rename(anthrome = baseline) %>%
    ...
    count(time_step, class,  wt = land_area, name = 'land_area', .drop = FALSE) %>% .[1500:1575,]
```
...
nmh4_summary <- contemp_vars %>%
  mutate(nmh4_km2 = nmh4 * land_area) %>%
  select(id, nmh4_km2) %>%
...
global_summary_lower %>% 
  select(-period, -year) %>% 
  pivot_wider(id_cols = class, names_from = time_step, values_from = percent_land_area) %>%
  mutate(across(where(is.numeric), ~replace_na(.x, 0))) %>%
  left_join(anthrome_key, by = 'class') %>%
  select(class, level, type, any_of(time_steps_ordered)) %>%
  write_csv('data/derived_data/an12_dgg_summary_global_lower.csv')

global_summary_upper %>% 
  select(-period, -year) %>% 
  pivot_wider(id_cols = class, names_from = time_step, values_from = percent_land_area) %>%
  mutate(across(where(is.numeric), ~replace_na(.x, 0))) %>%
  left_join(anthrome_key, by = 'class') %>%
  select(class, level, type, any_of(time_steps_ordered)) %>%
  write_csv('data/derived_data/an12_dgg_summary_global_upper.csv')
...
regional_summary_lower %>%
  ...
  select(region_name, class, level, type, any_of(time_steps_ordered)) %>%
  mutate(across(where(is.numeric), ~replace_na(.x, 0))) %>%
  write_csv('data/derived_data/an12_dgg_summary_region_lower.csv')

regional_summary_upper %>%
  select(-period, -year) %>%
  pivot_wider(id_cols = c(class, region_name), names_from = time_step, values_from = percent_land_area) %>%
  arrange(region_name, class) %>%
  left_join(anthrome_key, by = 'class') %>%
  select(region_name, class, level, type, any_of(time_steps_ordered)) %>%
  mutate(across(where(is.numeric), ~replace_na(.x, 0))) %>%
  write_csv('data/derived_data/an12_dgg_summary_region_upper.csv')
...
biome_summary_lower %>%
  ...
  select(biome, class, level, type, any_of(time_steps_ordered)) %>%
  mutate(across(where(is.numeric), ~replace_na(.x, 0))) %>%
  write_csv('data/derived_data/an12_dgg_summary_biomes_lower.csv')
...
biome_summary_upper %>%
  ...
  select(biome, class, level, type, any_of(time_steps_ordered)) %>%
  mutate(across(where(is.numeric), ~replace_na(.x, 0))) %>%
  write_csv('data/derived_data/an12_dgg_summary_biomes_upper.csv')

country_summary %>%
  select(-period, -year) %>%
  pivot_wider(id_cols = c(country_name, class), names_from = time_step, values_from = percent_land_area) %>%
  arrange(country_name, class) %>%
  left_join(anthrome_key, by = 'class') %>%
  select(country_name, class, level, type, any_of(time_steps_ordered)) %>%
  mutate(across(where(is.numeric), ~replace_na(.x, 0))) %>%
  write_csv('data/derived_data/an12_dgg_summary_country.csv')

country_summary_lower %>%
  select(-period, -year) %>%
  pivot_wider(id_cols = c(country_name, class), names_from = time_step, values_from = percent_land_area) %>%
  arrange(country_name, class) %>%
  left_join(anthrome_key, by = 'class') %>%
  select(country_name, class, level, type, any_of(time_steps_ordered)) %>%
  mutate(across(where(is.numeric), ~replace_na(.x, 0))) %>%
  write_csv('data/derived_data/an12_dgg_summary_country_lower.csv')

country_summary_upper %>%
  select(-period, -year) %>%
  pivot_wider(id_cols = c(country_name, class), names_from = time_step, values_from = percent_land_area) %>%
  arrange(country_name, class) %>%
  left_join(anthrome_key, by = 'class') %>%
  select(country_name, class, level, type, any_of(time_steps_ordered)) %>%
  mutate(across(where(is.numeric), ~replace_na(.x, 0))) %>%
  write_csv('data/derived_data/an12_dgg_summary_country_upper.csv')
...
legacy_mods <-
  ...
  mutate(
    ...
    nmh_aic = pmap_dbl(list(predictands, predictors, fam), calc_aic, 
                       dat = filter(regression_dat, (nmh3 + nmh4) > 0.5)),
    # protected areas
