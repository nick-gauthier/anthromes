...
hyde_inputs[['urban']] %>% range(na.rm = TRUE)

fixed_inputs <- list.files('data/raw_data/fixed_input', full.names = TRUE) %>%
  .[c(1, 4, 5)] %>%# remove greg28_cr.asc, iso_cr.asc
  read_stars() %>%
  setNames(c('pot_veg', 'area', 'pot_vill')) %>%
  mutate(trees = if_else(pot_veg <= 8, TRUE, FALSE)) # Define the non-woody classes in the pot_veg map
```
...
filter(!is.na(urban)) %>%
  mutate(anthrome = case_when(
      ...
      area > 0 & pot_veg != 15 & trees ~ 61,
      area > 0 & pot_veg != 15 & !trees ~ 62,
      area > 0 & pot_veg == 15 & used > 0 ~ 62, # in python code but not paper
      area > 0 & pot_veg == 15 ~ 63,
      TRUE ~ 70)
      ) %>%
  #select(x, y, anthrome) %>%
