...
test <- suppressWarnings(list.files('~/Downloads/upper/zip/', recursive = TRUE, full.names = TRUE) %>%  str_subset(., 'cropland') %>% map(raster) %>% brick %>%`crs<-`(value = '+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0') %>%
  exact_extract(dgg_land, 'sum'))

# check that ncol(hyde_test) == length(hyde_names) * length(time_steps)

hyde_dgg1 <- readRDS('hyde_dgg_upper')
hyde_dgg1;test

bind_cols(hyde_dgg1, test) %>%
  select(!contains('...'))

ncol(hyde_dgg1)

#missing 32
map(hyde_names, ~select(hyde_dgg1, contains(.)) %>% ncol)
test2 <- select(hyde_dgg1, contains('cropland')) %>% names

saveRDS(bind_cols(hyde_dgg1, select(test, !all_of(test2))), 'hyde_dgg_upper')
```
...
```{r}
ggplot(filter(anthromes_dgg_shp, id < 200000)) +
