...
Create a land mask based on HYDE 3.2. Get the cell centroids, and use to select the corresponding Level 12 DGG cells.
```{r}
land_area <- raster('data/raw_data/fixed_input/maxln_cr.asc') %>% 
  rasterToPoints()

dgg_ids <- unique(dgGEO_to_SEQNUM(dggs, land_area[,'x'], land_area[,'y'])$seqnum) 
```
...
Create polygons from the DGG system and locations
```{r}
dgg_land <- dgcellstogrid(dggs, dgg_ids, frame = FALSE, wrapcells = FALSE) %>%
  st_as_sf(crs = 4326) %>%
  mutate(dgg_id = dgg_ids) %>%
