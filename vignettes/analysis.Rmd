...
ggsave('contemp_summary_levels.png', width = 10,height = 6)
```
...
contemp %>%
  select(id, level, max, v_thr, v_rich) %>%
...
regression_dat <- an12_dgg %>%
  ...
  left_join(contemp_vars, by = 'id') %>%
  mutate(pa = pa_km2 > 0, ind = ind_cnt > 0, kba = kba_km2 > 0, cond3 = as.factor(X.3_cond_v4), v_rich = round(v_rich), v_thr = round(v_thr)) %>%
