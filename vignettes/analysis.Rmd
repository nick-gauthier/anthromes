...
region_summary <- an12_dgg %>% 
  ...
  pivot_longer(-c(id, land_area, region), names_to = 'time_step', values_to = 'anthrome') %>%
  left_join(anthrome_key, by = 'anthrome') %>% 
  select(time_step, region, land_area, level) %>%
  mutate(region = as.factor(region)) %>%
  count(time_step, level, region, wt = land_area, name = 'land_area', .drop = FALSE) %>%
  mutate(percent_land_area = land_area / total_land_area,
         region = as.numeric(as.character(region))) %>%
  left_join(time_key, by = 'time_step') %>%
  mutate(period = case_when(year %in% c(seq(-10000, -1000, 1000), 1) ~ 'Millennia',
         ...
         period = factor(period, c('Millennia', 'Centuries', 'Decades'))) %>%
    left_join(region_key) %>%
  select(time_step, region_name, level, percent_land_area, period, year)
...
region_summary %>% 
 ggplot(aes(as.factor(year), percent_land_area)) +
  ...
  scale_x_discrete(breaks = c(-10000, -5000, 1, 500, 1000, 1700, 1800, 1900, 2000),
                   labels = c('10000 ʙᴄᴇ', '5000 ʙᴄᴇ', '1 ᴄᴇ', '500 ᴄᴇ', '1000 ᴄᴇ', '1700 ᴄᴇ', '1800 ᴄᴇ', '1900 ᴄᴇ', '2000 ᴄᴇ')) +
  labs(x = 'Year', y = NULL) +
  theme(legend.position = 'none', strip.background.y = element_blank(), strip.text.y.left = element_text(angle = 0, hjust = 1), axis.line.y.left = element_blank(), axis.ticks.y = element_blank(), axis.text.y = element_blank())
...
biome_summary <- an12_dgg %>% 
  ...
  pivot_longer(-c(id, land_area, pot_veg), names_to = 'time_step', values_to = 'anthrome') %>%
  left_join(anthrome_key) %>%
  select(land_area, class, pot_veg, time_step) %>%
    mutate(pot_veg = as.factor(pot_veg)) %>%
  count(time_step, class, pot_veg, wt = land_area, name = 'land_area', .drop = FALSE) %>%
  mutate(percent_land_area = land_area / total_land_area,
         pot_veg = as.numeric(as.character(pot_veg))) %>%
  left_join(time_key, by = 'time_step') %>%
  mutate(period = case_when(year %in% c(seq(-10000, -1000, 1000), 1) ~ 'Millennia',
         ...
         period = factor(period, c('Millennia', 'Centuries', 'Decades'))) %>%
    left_join(biome_key, by = c('pot_veg' = 'biome_value')) %>%
  select(time_step, pot_veg, biome, class, percent_land_area, period, year)
...
bio <- biome_summary %>%
  filter(biome != 'Ice, snow', level != 'NODATA') %>%
 # mutate(pretty_biomes = str_wrap(biomes)) %>%
 ggplot(aes(as.factor(year), percent_land_area)) +
  geom_col(aes(fill = level, color = level))+ 
  ...
  scale_x_discrete(breaks = c(-10000, -5000, 1, 500, 1000, 1700, 1800, 1900, 2000),
                   labels = c('10000 ʙᴄᴇ', '5000 ʙᴄᴇ', '1 ᴄᴇ', '500 ᴄᴇ', '1000 ᴄᴇ', '1700 ᴄᴇ', '1800 ᴄᴇ', '1900 ᴄᴇ', '2000 ᴄᴇ')) +
  labs(x = 'Year', y = NULL) +
  theme(legend.position = 'bottom', strip.background.y = element_blank(), strip.text.y.left = element_text(angle = 0, hjust = 1), axis.line.y.left = element_blank(), axis.ticks.y = element_blank(), axis.text.y = element_blank())

ggsave('anthromes_levels_biomes.png', height = 6, width = 9)
```
