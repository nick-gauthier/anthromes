...
#   mutate(Cultured = Wild + Cultured) %>% # to match the stacked bar plot
#   select(time_step, Wild, Cultured, period) %>%
#   pivot_longer(Wild:Cultured, names_to = 'type', values_to = 'percent_land_area') %>%
...
global_types <- global_summary %>%
  ...
  mutate(type = if_else(type == 'Cultured', 'Intensive', 'Wild')) # just for the colors
...
ggsave('figures/anthromes_summary_region.png', plot = addSmallLegend(region_plot) + guides(color = FALSE), height = 8, width = 9, dpi = 600)
...
```{r}
ggsave('figures/anthromes_legacies_regions.png', height = 8, width = 7.2)
...
```{r}
ggsave('figures/olson_biomes.png', height = 10, width = 9)
```
...
```{r}
ggsave('figures/biomes15.png', height = 10, width = 9)
```
