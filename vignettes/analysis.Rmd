...
global_plot <- anthromes_summary %>%
 ...
 ggplot(aes(time_step, percent_land_area)) +
  ...
  theme(legend.position = 'bottom', strip.text.x = element_text(hjust = 0), panel.spacing.x=unit(.1, "lines"), strip.background = element_blank(), axis.line.y = element_blank(),  axis.text=element_text(size=rel(0.7)), panel.border = element_rect(colour = "grey20", fill=NA, size = 1))

addSmallLegend(global_plot)
```


```{r}
ggsave('anthrome_global_summary.svg', plot = addSmallLegend(global_plot), width = 9, height = 3.5)
```

```{r}
global_plot / region_plot  + plot_layout(heights = c(1,3))  + plot_annotation(tag_levels = 'a', tag_prefix = '(', tag_suffix = ')') & theme(legend.position = 'none')
```
...
```{r}
ggsave('anthromes_summary_region.svg', plot = addSmallLegend(region_plot), height = 8, width = 9)
```
...
biome_plot <- biome_summary %>% 
    mutate(time_step = ordered(time_step, levels = time_steps_ordered)) %>%
  filter( class != 'NODATA', class != 'Ice, uninhabited', biome != 'Ice, snow') %>%
 ggplot(aes(time_step, percent_land_area)) +
  geom_col(aes(fill = class, color = class), width = .8, size = .1)+
      geom_vline(xintercept = c('0AD', '1700AD'), color = 'black', alpha = .5) +
        geom_hline(yintercept = .5, linetype = 2, color = 'black', alpha = .25) +
...
```{r}
ggsave('anthromes_summary_biome.svg', plot =addSmallLegend(biome_plot), height = 8, width = 9)
```

## Figure 4 - Contemporary Data

```{r}
cons_plot <- bind_rows(kba_summary,pa_summary, ind_summary, unused_summary, nmh3_summary, nmh4_summary, shared_3gc_summary, wild_3gc_summary, .id = 'type') %>%
  mutate(type = factor(case_when(type == 1 ~ 'Key Biodiversity Areas',
      ...
      mutate(time_step = ordered(time_step, levels = time_steps_ordered)) %>%
  filter(class != 'NODATA', class != 'Ice, uninhabited') %>%
  mutate(var = var / total_land_area) %>%
 ggplot(aes(time_step, var)) +
   geom_col(aes(fill = class, color = class), width = .8, size = .1)+
      geom_vline(xintercept = c('0AD', '1700AD'), color = 'black', alpha = .5) +
  #      geom_hline(yintercept = .5, linetype = 2, color = 'black', alpha = .25) +
   facet_wrap(~type, ncol = 2, dir = 'h', scales = 'free_y') +
  scale_fill_manual(values = anthrome_class_color, name = 'Anthrome') +
    scale_color_manual(values = anthrome_class_color, name = 'Anthrome') +
   theme_classic() +
  scale_y_continuous(expand = c(0.01,0.01), labels = scales::percent_format(accuracy = 1), n.breaks = 3) +
...
legacy_plot <- (wrap_elements(full = addSmallLegend(cons_plot)) / (global_legacy_plot/natural_legacy_plot + plot_layout(guides = 'collect'))) + plot_layout(heights = c(2,1)) + plot_annotation(tag_levels = 'a', tag_prefix = '(', tag_suffix = ')')

legacy_plot
```

```{r}
ggsave('anthromes_legacies.svg', plot =legacy_plot, height = 8, width = 8)
```
