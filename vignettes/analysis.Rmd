...
biome_key <- tibble(biome_value = 1:15) %>%
  mutate(biome = case_when(biome_value == 1 ~ 'Tropical evergreen woodland',
         ...
         biome = factor(biome, levels = c('Tropical evergreen woodland', 'Tropical deciduous woodland', 'Temperate woodland', 'Boreal woodland', 'Mixed woodland', 'Grassland, savanna, steppe',	'Tundra',	'Desert and barren', 'Ice, snow')))
```
...
a <- addSmallLegend(a)
ggsave(a, 'anthrome_areas_population.png', width = 9, height = 3.5)
```
...
biome_summary %>%
 ...
 ggplot(aes(as.factor(year), percent_land_area)) +
  ...
  scale_x_discrete(breaks = c(-10000, -5000, 1, 500, 1000, 1700, 1800, 1900, 2000),
                   labels = c('10000 ʙᴄᴇ', '5000 ʙᴄᴇ', '1 ᴄᴇ', '500 ᴄᴇ', '1000 ᴄᴇ', '1700 ᴄᴇ', '1800 ᴄᴇ', '1900 ᴄᴇ', '2000 ᴄᴇ')) +
  labs(x = 'Year', y = expression('Percent land surface')) + theme(legend.position = 'none')
