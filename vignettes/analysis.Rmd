...
regression_dat <- an12_dgg %>%
  ...
  left_join(contemp_vars, by = 'id') %>%
  mutate(pa = pa_km2 > 0, ind = ind_cnt > 0, cond3 = as.factor(X.3_cond_v4)) %>%
...
test_aic_class <- expand_grid(time_step = time_steps_centuries[-22],
                   predictands = c('mean', 'max', 'v_thr', 'v_rich', 'food_cal')) %>%
  mutate(predictors = paste0('`', time_step, '`'),
         aic = map2_dbl(predictands, predictors, calc_aic)) %>%
  left_join(time_key)
...
test_aic_class3 <- expand_grid(time_step = time_steps_centuries[-22],
                   predictands = c('cond3', 'nmh', 'landsuit')) %>%
  mutate(predictors = paste0('`', time_step, '`'),
         aic = map2_dbl(predictands, predictors, calc_aic3)) %>%
  left_join(time_key)
...
bind_rows(test_aic_class, test_aic_class2, test_aic_class3)%>%
  group_by(predictands) %>%
  mutate(aic_scaled = scales::rescale(aic)) %>%
  ggplot(aes(as.factor(year), predictands, fill = aic_scaled), color = 'white') +
  geom_tile() +
  scale_fill_viridis_c(direction = -1) +
  theme_classic() +
...
ggsave('anthrome_legacies.png', height = 3, width = 6)
```
...
hist(log(contemp_vars$food_cal))
