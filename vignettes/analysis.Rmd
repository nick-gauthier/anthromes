...
anthromes_summary <- an12_dgg %>% 
  ...
  select(-pot_veg, -pot_vill, -region_name) %>%
...
contemp_summary <- bind_rows(kba_summary,pa_summary, ind_summary, unused_summary, nmh3_summary, nmh4_summary, shared_3gc_summary, wild_3gc_summary, .id = 'type') %>%
  mutate(type = factor(case_when(type == 1 ~ 'Key Biodiversity Areas',
                          type == 2 ~ 'Protected Areas',
                          type == 3 ~ 'Indigenous Lands',
                          type == 4 ~ 'Seminatural and Wildlands (2017)',
                          type == 5 ~ 'Potential Natural (NMH)',
                          type == 6 ~ 'Likely Natural (NMH)',
                          type == 7 ~ 'Shared Lands (3GC)',
                          type == 8 ~ 'Large Wild Areas (3GC)'), levels = c('Key Biodiversity Areas','Protected Areas', 'Indigenous Lands', 'Seminatural and Wildlands (2017)', 'Shared Lands (3GC)', 'Large Wild Areas (3GC)', 'Potential Natural (NMH)',  'Likely Natural (NMH)')))
```
...
```{r}
anthromes_summary %>% 
  select(-period, -anthrome) %>% 
  pivot_wider(id_cols = c(time_step, year), names_from = class, values_from = percent_land_area) %>%
  arrange(year) %>%
  mutate(across(everything(), ~replace_na(.x, 0))) %>%
  write_csv('data/derived_data/an12_dgg_summary.csv')

region_summary %>%
  select(-period) %>%
  pivot_wider(id_cols = c(time_step, year, region_name), names_from = class, values_from = percent_land_area) %>%
  arrange(year) %>%
  mutate(across(everything(), ~replace_na(.x, 0))) %>%
...
biome_summary %>%
  select(-period) %>%
 pivot_wider(id_cols = c(time_step, year, biome), names_from = class, values_from = percent_land_area) %>%
  arrange(year) %>%
  mutate(across(everything(), ~replace_na(.x, 0))) %>%
...
contemp_summary %>%
  select(-period) %>%
 pivot_wider(id_cols = c(time_step, year, type), names_from = class, values_from = var) %>%
  arrange(year) %>%
  mutate(across(everything(), ~replace_na(.x, 0))) %>%
...
global_plot <- anthromes_summary %>%
 ...
 ggplot(aes(time_step, percent_land_area)) +
    geom_col(aes(fill = class, color = class), position = 'fill', width = .8, size = .1)+
      ...
      geom_line(data = hyde_pop_all, aes(y = pop_percent), color = 'darkred', linetype = 1, group = 1) +
 ...
 ggplot(aes(time_step, percent_land_area)) +
  geom_col(aes(fill = class, color = class), width = .8, size = .1)+
      geom_vline(xintercept = c('0AD', '1700AD'), color = 'black', alpha = .5) +
        geom_hline(yintercept = .5, linetype = 2, color = 'black', alpha = .25) +
      geom_line(data = hyde_pop_regions, aes(y = pop_percent), color = 'darkred', linetype = 1, group = 1) +
...
contemp_plot <- contemp_summary %>%
 ...
 ggplot(aes(time_step, var)) +
   geom_col(aes(fill = class, color = class), position = 'fill', width = .8, size = .1)+
      geom_vline(xintercept = c('0AD', '1700AD'), color = 'black', alpha = .5) +
       geom_hline(yintercept = .5, linetype = 2, color = 'black', alpha = .25) +
   facet_wrap(~type, ncol = 2, dir = 'h') +
  scale_fill_manual(values = anthrome_class_color, name = 'Anthrome') +
    scale_color_manual(values = anthrome_class_color, name = 'Anthrome') +
   theme_classic() +
  scale_y_continuous(expand = c(0.01,0.01), labels = scales::percent_format(accuracy = 1), n.breaks = 3) +
  scale_x_discrete(breaks = c('8000BC', '0AD', '800AD', '1700AD', '1800AD', '1900AD', '2000AD'), 
                   labels = c('8000ʙᴄᴇ', '1ᴄᴇ', '800ᴄᴇ', '1700ᴄᴇ', '1800ᴄᴇ', '1900ᴄᴇ', '2000ᴄᴇ')) +
  labs(x = 'Year', y = 'Percent land surface') +
  theme(strip.text.x = element_text(hjust = 0), panel.spacing.x=unit(.8, "lines"), strip.background = element_blank(), axis.line.y = element_blank(),  axis.text=element_text(size=rel(0.7)), panel.border = element_rect(colour = "grey20", fill=NA, size = 1))

addSmallLegend(contemp_plot)
```
...
global_legacy_plot <- bind_rows(global_legacy_glm1,global_legacy_glm2) %>%
  ...
  scale_fill_viridis_c(direction = -1, name = 'Predictive ability\n(scaled AIC)', breaks = c(.05, .95), labels = c('Best', 'Worst')) +
  theme_classic() +
...
legacy_plot <- (wrap_elements(full = addSmallLegend(contemp_plot)) / (global_legacy_plot/natural_legacy_plot + plot_layout(guides = 'collect'))) + plot_layout(heights = c(2,1)) + plot_annotation(tag_levels = 'a', tag_prefix = '(', tag_suffix = ')')
...
ggsave('figures/anthromes_legacies.svg', plot = legacy_plot, height = 8, width = 8)
...
ggsave('figures/anthromes_legacies.png', plot = legacy_plot, height = 8, width = 8, dpi = 600)
```
...
e <- bind_rows(test_aic_class, test_aic_class2) %>%
  ...
  labs(x = 'Predictor Year', y = NULL)
e
ggsave(e, 'figures/anthrome_legacies34.png', height = 2, width = 6)
```
