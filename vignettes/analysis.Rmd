...
region_summary %>% 
 ...
 ggplot(aes(as.factor(year), land_area)) +
  geom_col(aes(fill = class), color = 'white', size = .1)+
   facet_wrap(~region_name, scales = 'free_y', ncol= 1) +
  scale_fill_manual(values = anthrome_class_color, name = 'Anthrome') +
    geom_vline(xintercept = c('1', '1700'), linetype = 2, color = 'red') +
   theme_classic() +
  scale_x_discrete(breaks = c(-8000, 1, 500, 1500, 1800, 1900, 2000),
                   labels = c('8000 ʙᴄᴇ', '1 ᴄᴇ', '500 ᴄᴇ', '1500 ᴄᴇ', '1800 ᴄᴇ', '1900 ᴄᴇ', '2000 ᴄᴇ')) +
  labs(x = 'Year', y = NULL) +
  theme(legend.position = 'none', strip.text.x = element_text(hjust = 0 ), strip.text.y.left = element_text(angle = 0, hjust = 1), panel.spacing.y=unit(.25, "lines"), panel.spacing.x=unit(0.2, "lines"), strip.background = element_blank(), axis.line.y = element_blank(), axis.text.y = element_blank(), axis.ticks.y = element_blank())

region_summary %>% 
  filter(class != 'Ice, uninhabited') %>%
 ggplot(aes(as.factor(year), percent_land_area)) +
  geom_col(aes(fill = class), color = 'white', size = .1)+
    geom_line(data = hyde_pop_regions, aes(x = as.factor(year), y = pop_percent), color = 'maroon', linetype = 1, group = 1, size = 1) +
   facet_grid(region_name~period, scales = 'free', space = 'free_x', switch = 'y') +
  scale_fill_manual(values = anthrome_class_color, name = 'Anthrome') +
   theme_classic() +
  scale_x_discrete(breaks = c(-8000, 1, 500, 1500, 1800, 1900, 2000),
                   labels = c('8000 ʙᴄᴇ', '1 ᴄᴇ', '500 ᴄᴇ', '1500 ᴄᴇ', '1800 ᴄᴇ', '1900 ᴄᴇ', '2000 ᴄᴇ')) +
  labs(x = 'Year', y = NULL) +
  theme(legend.position = 'none', strip.text.x = element_text(hjust = 0 ), strip.text.y.left = element_text(angle = 0, hjust = 1), panel.spacing.y=unit(.25, "lines"), panel.spacing.x=unit(0.1, "lines"), strip.background = element_blank(), axis.line.y = element_blank(), axis.text.y = element_blank(), axis.ticks.y = element_blank())

ggsave('anthromes_summary_region.png', height = 6, width = 6)
...
biome_summary %>%
  filter(biome != 'Ice, snow', class != 'NODATA') %>%
 ggplot(aes(as.factor(year), percent_land_area)) +
  ...
  scale_x_discrete(breaks = c(-8000, 1, 500, 1500, 1800, 1900, 2000),
                   labels = c('8000 ʙᴄᴇ', '1 ᴄᴇ', '500 ᴄᴇ', '1500 ᴄᴇ', '1800 ᴄᴇ', '1900 ᴄᴇ', '2000 ᴄᴇ')) +
  labs(x = 'Year', y = NULL) +
  theme(legend.position = 'none', strip.background = element_rect(fill = NA, colour = "black"), strip.text.y.right = element_text(angle = 0, hjust = 0), axis.line.y.left = element_blank(), axis.ticks.y = element_blank(), axis.text.y = element_blank(), panel.spacing.y=unit(.25, "lines"), panel.spacing.x=unit(.25, "lines"))

ggsave('anthromes_class_biomes.png', height = 5, width = 6.5)
```
...
biome_summary %>% 
 ...
 ggplot(aes(as.factor(year), percent_land_area)) +
  geom_col(aes(fill = class), color = 'white', size = .1)+
    geom_line(data = hyde_pop_biomes, aes(x = as.factor(year), y = pop_percent), color = 'maroon', linetype = 1, group = 1, size = 1) +
   facet_grid(biome~period, scales = 'free', space = 'free_x', switch = 'y') +
  scale_fill_manual(values = anthrome_class_color, name = 'Anthrome') +
   theme_classic() +
  scale_x_discrete(breaks = c(-8000, 1, 500, 1500, 1800, 1900, 2000),
                   labels = c('8000 ʙᴄᴇ', '1 ᴄᴇ', '500 ᴄᴇ', '1500 ᴄᴇ', '1800 ᴄᴇ', '1900 ᴄᴇ', '2000 ᴄᴇ')) +
...
ggsave('anthromes_summary_biome.png', height = 5, width = 6.5)
```
...
kba_summary <- contemp_vars %>%
  ...
  pivot_longer(-c(id, kba_km2), names_to = 'time_step', values_to = 'anthrome') %>%
    left_join(anthrome_key, by = 'anthrome') %>% 
  select(id, time_step, kba_km2, level) %>%
  count(time_step, level, wt = kba_km2, name = 'var', .drop = FALSE) %>% 
  left_join(time_key, by = 'time_step') %>%
  mutate(period = case_when(year %in% c(seq(-10000, -1000, 1000), 1) ~ 'Millennia',
                            year %in% seq(100, 1700, 100) ~ 'Centuries',
                            year %in%  c(seq(1710, 2010, 10), 2017) ~ 'Decades'),
         period = factor(period, c('Millennia', 'Centuries', 'Decades'))) %>%
  select(time_step, level, var, period, year)
...
pa_summary <- contemp_vars %>%
  ...
  pivot_longer(-c(id, pa_km2), names_to = 'time_step', values_to = 'anthrome') %>%
   left_join(anthrome_key, by = 'anthrome') %>% 
  select(id, time_step, pa_km2, level) %>%
  count(time_step, level, wt = pa_km2, name = 'var', .drop = FALSE) %>% 
  left_join(time_key, by = 'time_step') %>%
  mutate(period = case_when(year %in% c(seq(-10000, -1000, 1000), 1) ~ 'Millennia',
                            year %in% seq(100, 1700, 100) ~ 'Centuries',
                            year %in%  c(seq(1710, 2010, 10), 2017) ~ 'Decades'),
...
nmh4_summary <- contemp_vars %>%
  ...
  pivot_longer(-c(id, nmh4_km2), names_to = 'time_step', values_to = 'anthrome') %>%
    left_join(anthrome_key, by = 'anthrome') %>% 
  select(id, time_step, nmh4_km2, level) %>%
  count(time_step, level, wt = nmh4_km2, name = 'var', .drop = FALSE) %>% 
  left_join(time_key, by = 'time_step') %>%
  mutate(period = case_when(year %in% c(seq(-10000, -1000, 1000), 1) ~ 'Millennia',
                            year %in% seq(100, 1700, 100) ~ 'Centuries',
                            year %in%  c(seq(1710, 2010, 10), 2017) ~ 'Decades'),
...
nmh3_summary <- contemp_vars %>%
  ...
  pivot_longer(-c(id, nmh3_km2), names_to = 'time_step', values_to = 'anthrome') %>%
    left_join(anthrome_key, by = 'anthrome') %>% 
  select(id, time_step, nmh3_km2, level) %>%
  count(time_step, level, wt = nmh3_km2, name = 'var', .drop = FALSE) %>% 
  left_join(time_key, by = 'time_step') %>%
  mutate(period = case_when(year %in% c(seq(-10000, -1000, 1000), 1) ~ 'Millennia',
                            year %in% seq(100, 1700, 100) ~ 'Centuries',
                            year %in%  c(seq(1710, 2010, 10), 2017) ~ 'Decades'),
...
shared_3gc_summary <- contemp_vars %>%
  ...
  pivot_longer(-c(id, land_ar), names_to = 'time_step', values_to = 'anthrome') %>%
    left_join(anthrome_key, by = 'anthrome') %>% 
  select(id, time_step, land_ar, level) %>%
  count(time_step, level, wt = land_ar, name = 'var', .drop = FALSE) %>% 
  left_join(time_key, by = 'time_step') %>%
  mutate(period = case_when(year %in% c(seq(-10000, -1000, 1000), 1) ~ 'Millennia',
                            year %in% seq(100, 1700, 100) ~ 'Centuries',
                            year %in%  c(seq(1710, 2010, 10), 2017) ~ 'Decades'),
...
```{r}
a <- bind_rows(kba_summary,pa_summary, nmh3_summary, nmh4_summary, shared_3gc_summary, wild_3gc_summary, .id = 'type') %>%
  mutate(type = factor(case_when(type == 1 ~ 'Key Biodiversity Areas',
                          type == 2 ~ 'Protected Areas',
                          type == 3 ~ 'Potential Natural (NMH)',
                          type == 4 ~ 'Likely Natural (NMH)',
                          type == 5 ~ 'Shared Lands (3GC)',
                          type == 6 ~ 'Large Wild Areas (3GC)'), levels = c('Key Biodiversity Areas','Protected Areas','Shared Lands (3GC)', 'Large Wild Areas (3GC)', 'Potential Natural (NMH)',  'Likely Natural (NMH)'))) %>%
  filter(level != 'NODATA') %>%
  mutate(var = var / total_land_area) %>%
 ggplot(aes(as.factor(year), var)) +
#  geom_area(aes(fill = class, color = class), position = 'stack') +
#  geom_col(aes(fill = class), color = 'lightgrey', size = .2)+
  geom_col(aes(fill = level, color = level))+
  #geom_hline(yintercept = c(.25, .50, .75), linetype = 2, alpha = .3) +
 facet_grid(type~period, scales = 'free', space = 'free_x', switch = 'y') + #, labeller = label_wrap_gen(width=15)) +
  scale_color_manual(values = anthrome_level_color, name = 'Anthrome') +
  scale_fill_manual(values = anthrome_level_color, name = 'Anthrome') +
      theme_bw() +
 # scale_y_continuous(labels = scales::percent, breaks= scales::breaks_extended(n=3, w = c(.45, .05, .25, .2))) +
  scale_x_discrete(breaks = c(-8000, 1, 500, 1500, 1800, 1900, 2000),
                   labels = c('8000 ʙᴄᴇ', '1 ᴄᴇ', '500 ᴄᴇ', '1500 ᴄᴇ', '1800 ᴄᴇ', '1900 ᴄᴇ', '2000 ᴄᴇ')) +
  labs(x = 'Year', y = NULL) +
  theme(panel.spacing = unit(.25, "lines"),
        axis.line.y = element_blank(),  axis.text.y = element_blank(), axis.ticks.y = element_blank(), panel.grid.major.y = element_blank(), panel.grid.minor = element_blank(), strip.background = element_blank(), strip.text.y.left = element_text(angle = 0, hjust = 1))
a

ggsave('anthrome_areas_conservation.png', width = 6, height = 3.25)
```
...
```{r}
qplot(year, aic, data = test_aic_class, geom = 'point')
```
...
m1 <- glm(v_rich ~ `1500AD`, family = poisson, data = regression_dat)
m2 <- glm(v_rich ~ `2000AD`, family = poisson, data = regression_dat)
m3 <- glm(v_rich ~ `1500AD` + `2000AD`, family = poisson, data = regression_dat)
m4 <- glm(v_rich ~ `1500AD` * `2000AD`, family = poisson, data = regression_dat)
...
st_crop(st_bbox(c(xmin = -100, xmax = -50, ymin = -10, ymax = 20))) %>% # comment out for whole world
  ggplot() +
  geom_sf(aes(fill = pred1500), color = NA) +
...
```{r}
test <- regression_dat %>%
  modelr::add_predictions(m1, var = 'pred1500',type = 'response') %>%
