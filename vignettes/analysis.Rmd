...
plot(grid)
ggplot(grid, aes(long,lat,group=group)) +
  geom_polygon() +
  geom_path(color="white") +
  coord_map('ortho')

sf::st_as_sf(grid, coords = c("long", "lat")) %>% 
  group_by(cell, piece) %>% 
...
ggplot(grid) +
  ...
  theme_void() +
    theme_void()#+
 # coord_sf(crs= "+proj=ortho +lat_0=20 +lon_0=-10") # won't work if dateline is wrapped
```
# Trajectory Clusters
...
set.seed(1000)
cluster_samp <- an12_dgg %>%
  slice_sample(prop = 0.01, weight_by = an12_dgg_inputs$land_area) %>% 
  pivot_longer(-id, names_to = 'time_step', values_to = 'anthrome') %>%
  dplyr::mutate(time_step = factor(time_step, levels = time_steps_ordered)) %>%
   dplyr::arrange(id, time_step) %>%
  left_join(anthrome_key) %>%
  select(id, time_step, level)


cluster_samp_millennia <- cluster_samp %>%
  filter(!(time_step %in% recent_time_steps),
         time_step %in% time_steps_millennia) %>%
  pivot_wider(names_from = time_step, values_from = level)

cluster_samp_centuries <- cluster_samp %>%
  filter(!(time_step %in% recent_time_steps),
         time_step %in% time_steps_centuries) %>%
  pivot_wider(names_from = time_step, values_from = level)

cluster_samp_decades <- cluster_samp %>%
  filter(!(time_step %in% recent_time_steps),
         time_step %in% time_steps_decades) %>%
  pivot_wider(names_from = time_step, values_from = level)

clusters <- list(cluster_samp_millennia, cluster_samp_centuries, cluster_samp_decades) %>%
  map(~select(., -id) %>% nomclust(prox = FALSE))

```

```{r}
#arrange this by latitide!
cluster_samp_centuries %>%
  mutate(cluster = clusters[[1]]$mem$clu_4) %>%
  pivot_longer(-c(id, cluster), names_to = 'time_step', values_to = 'level') %>%
                   dplyr::mutate(time_step = factor(time_step, levels = time_steps_ordered)) %>%
     dplyr::mutate(time_step = ordered(time_step, levels = time_steps_ordered)) %>%
  left_join(time_key) %>%
  ggplot(aes(year, as.character(id))) +
...
test.dat <- cluster_samp %>%
  filter(!(time_step %in% recent_time_steps),
         time_step %in% time_steps_centuries) %>%
       dplyr::mutate(time_step = factor(time_step, levels = time_steps_ordered, ordered = TRUE)) %>%
  left_join(time_key) %>%
  mutate(level = factor(level, levels = rev(c('Dense settlements', 'Villages', 'Croplands',
            'Rangelands', 'Seminatural', 'Wildlands')), ordered = TRUE),
         id = as.factor(id),
         anthrome_level = as.numeric(level))
  
test_gam <- bam(anthrome_level ~ s(year, id, bs = 'fs', k = 5) , data = test.dat, family = ocat(R = 6), discrete = TRUE)
#s(year, bs = 'cr', m = 2) + s(year, by = id, bs = 'cs', m = 1) +  s(id, bs = 're')
plot(test_gam)
...
```{r}
cluster_samp_decades %>%
  mutate(cluster = clusters[[3]]$mem$clu_6) %>%
...
```{r}
cluster_samp_millennia %>%
  mutate(cluster = clusters[[1]]$mem$clu_6) %>%
    pivot_longer(-c(id, cluster), names_to = 'time_step', values_to = 'anthrome') %>%
                   dplyr::mutate(time_step = factor(time_step, levels = time_steps_ordered)) %>%
  group_by(cluster, time_step) %>%
  dplyr::summarise(anthrome = raster::modal(anthrome))

cluster_samp_centuries %>%
  mutate(cluster = clusters[[2]]$mem$clu_6) %>%
    pivot_longer(-c(id, cluster), names_to = 'time_step', values_to = 'anthrome') %>%
                   dplyr::mutate(time_step = factor(time_step, levels = time_steps_ordered)) %>%
  group_by(cluster, time_step) %>%
  dplyr::summarise(anthrome = raster::modal(anthrome))
...
cluster_samp %>%
   filter(!(time_step %in% recent_time_steps),
         time_step %in% time_steps_millennia) %>%
  pivot_wider(names_from = time_step, values_from = anthrome) %>%
  mutate(cluster = clusters_millennia$mem$clu_6) %>%
  pivot_longer(-c(id, cluster), names_to = 'time_step', values_to = 'anthrome') %>%
                   dplyr::mutate(time_step = factor(time_step, levels = time_steps_ordered)) %>%
  ggplot(aes(time_step, anthrome, group = id, color = as.factor(cluster))) +
  geom_line(alpha = .5, size = 1.2) +
...
cluster_samp %>%
   filter(!(time_step %in% recent_time_steps),
         time_step %in% time_steps_centuries) %>%
  pivot_wider(names_from = time_step, values_from = anthrome) %>%
  mutate(cluster = clusters_centuries$mem$clu_6) %>%
  pivot_longer(-c(id, cluster), names_to = 'time_step', values_to = 'anthrome') %>%
                   dplyr::mutate(time_step = factor(time_step, levels = time_steps_ordered)) %>%
  ggplot(aes(time_step, anthrome, group = id, color = as.factor(cluster))) +
  geom_line(alpha = .5, size = 1.2) +
...
cluster_samp %>%
   filter(!(time_step %in% recent_time_steps),
         time_step %in% time_steps_decades) %>%
  pivot_wider(names_from = time_step, values_from = anthrome) %>%
  mutate(cluster = clusters_decades$mem$clu_6) %>%
  pivot_longer(-c(id, cluster), names_to = 'time_step', values_to = 'anthrome') %>%
                   dplyr::mutate(time_step = factor(time_step, levels = time_steps_ordered)) %>%
  ggplot(aes(time_step, anthrome, group = id, color = as.factor(cluster))) +
  geom_line(alpha = .5, size = 1.2) +
