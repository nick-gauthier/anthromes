...
#  geom_col(aes(fill = class), color = 'lightgrey', size = .2)+
  ...
  geom_line(data = hyde_pop, aes(x = as.factor(year), y = pop_percent), group = 1) + 
  geom_label(aes(label = label), data = tibble(year = 1830,  percent_land_area = .1, label = 'Population', period = factor('Decades', c('Millennia', 'Centuries', 'Decades')))) +
...
```{r}
library(raster)
hfp <- raster::raster('data/raw_data/hfp2000_merisINT.tif')
#rainfed <- 
...
nmh_extract <- exact_extract(nmh, dgg, fun = 'mode')
...
```{r}
rainfed <- raster('../comb_low/w001001.adf')
rainfed[rainfed <= -800] <- NA
plot(rainfed)

rainfed_extract <- exact_extract(rainfed, dgg, fun = 'mode')

rainfed_dgg <- dgg %>%
  mutate(rainfed = as.factor(rainfed_extract))

ggplot(rainfed_dgg) +
  geom_sf(aes(fill = rainfed), color = NA) +
  scale_fill_viridis_d()
```
...
contemporary_vars <- read_csv('../three_conditions_v4_rep_data.csv') %>% 
  left_join(land_mask, by = c('l1_id' = 'L1_ID')) %>% 
  st_as_sf() %>%
  select(l1_id, ANL12_ID, `@3_cond_v4`, hfp_max, land_km2, pa_km2, ind_cnt, v_rich, v_thr)
```

```{r}
tc <- ggplot() +
  geom_sf(data = contemporary_vars , aes(fill = as.factor(`@3_cond_v4`)), color = NA) +
  scale_fill_viridis_d() +
  coord_sf(crs = st_crs("+proj=eck4"))
```

```{r}
pa <- contemporary_vars %>%
  mutate(pa_fact = cut(pa_km2, breaks = 10)) %>%
  ggplot() +
  geom_sf(aes(fill = pa_fact), color = NA) +
  scale_fill_viridis_d() +
    ggtitle('Protected area (km2)') +
  coord_sf(crs = st_crs("+proj=eck4"))
```

```{r}
hf <- contemporary_vars %>%
  mutate(hfp_fact = cut(hfp_max, breaks = 10)) %>%
  ggplot() +
  geom_sf(aes(fill = hfp_fact), color = NA) +
  scale_fill_viridis_d() +
...
```{r}
v_thr <- contemporary_vars %>%
  mutate(v_thr_fact = cut(v_thr, breaks = 10)) %>%
  ggplot() +
  geom_sf(aes(fill = v_thr_fact), color = NA) +
  scale_fill_viridis_d() +
  ggtitle('Threatened vertebrate species') +
  coord_sf(crs = st_crs("+proj=eck4"))
```

```{r}
olson <- ggplot() +
  geom_sf(data = contemporary_vars , aes(fill = as.factor(biome)), color = NA) +
  scale_fill_viridis_d() +
  ggtitle('Olson biome') +
  coord_sf(crs = st_crs("+proj=eck4"))
```
...
```{r}
ggsave('pa.png', pa, width = 12, height = 7)
ggsave('hf.png', hf, width = 12, height = 7)
ggsave('v_thr.png', v_thr, width = 12, height = 7)
ggsave('olson.png', olson, width = 12, height = 7)
ggsave('tgc.png', tc, width = 12, height = 7)


```

```{r}
contemporary_vars %>% st_crop(st_bbox(c(xmin = 40, xmax = 60, ymin = 40, ymax = 60))) %>%
  mutate(test = cut(pa_km2, breaks = 5)) %>%
  ggplot() +
  geom_sf(aes(fill = test), color = NA) +
  scale_fill_viridis_d() +
  coord_sf(crs = st_crs("+proj=eck4"))
...
```{r}
contemporary_vars %>% st_crop(st_bbox(c(xmin = 30, xmax = 40, ymin = 30, ymax = 40))) %>%
  ggplot() +
...
```{r}
biome_summary <- an12_dgg %>% 
  left_join(st_drop_geometry(  dplyr::select(contemporary_vars,ANL12_ID, biome)), by = c('id' = 'ANL12_ID')) %>%
  left_join(st_drop_geometry(an12_dgg_inputs), by = 'id') %>%
