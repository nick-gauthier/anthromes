...
onsets <- bind_rows(select(an12_dgg_inputs, id) %>% left_join(onset_seminatural), 
                    select(an12_dgg_inputs, id) %>% left_join(onset_used), .id = 'type') %>%
...
```{r}
write_sf(onsets, 'anthromes_onsets.csv')
```
