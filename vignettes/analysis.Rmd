...
total_land_area <- sum(fixed_inputs$area)
```

# Summary
```{r}
anthromes_summary <- anthromes_dgg %>% 
  left_join(fixed_inputs, by = 'id') %>%
  select(-pot_veg, -pot_vill, -trees) %>%
  pivot_longer(-c(id, area), names_to = 'time_step', values_to = 'anthrome') %>%
  count(time_step, anthrome, wt = area, name = 'area') %>%
  mutate(percent_land_area = area / total_land_area) %>%
