...
a <- contemp %>%
  ...
  summarise(protected = sum(pa_km2, na.rm = TRUE),
            ...
            all_land = sum(land_ar, na.rm = TRUE)) %>%
...
d <- contemp %>%
  ...
  ggplot(aes(X_3_c_4, n, fill = level)) +
  geom_col(position = 'fill') +
  scale_fill_manual(values = anthrome_level_color, name = 'Anthrome level') +
  coord_flip() +
       scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
...
a + c+d + plot_layout(guides = "collect") & theme(legend.position = "bottom")
...
bind_rows(test_aic_class, test_aic_class2) %>%
  ...
  labs(x = 'Predictor Year', y = NULL, subtitle = 'Predicted Variable')+
  coord_fixed() +
 theme(plot.subtitle = element_text(hjust = -.4))

ggsave('anthrome_legacies34.png', height = 3, width = 6)
```
