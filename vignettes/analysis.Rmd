...
an12_dgg_baseline <- read_anthromes('an12_dgg.csv')
an12_dgg_upper <- read_anthromes('tmp/an12_dgg_upper.csv')
an12_dgg_lower <- read_anthromes('tmp/an12_dgg_lower.csv')

an12_dgg <- an12_dgg_baseline %>%
  rename(baseline = anthrome) %>%
  mutate(upper = an12_dgg_upper$anthrome,
         lower = an12_dgg_lower$anthrome)%>%
  mutate(time_step = ordered(time_step, levels = time_steps_ordered), .after = id) %>%
...
regression_dat <- an12_dgg %>%
  filter(time_step %in% c(time_steps_millennia, time_steps_centuries, '2010AD', '2017AD')) %>%
...
Summary of global land area for each anthrome and broken down by region and biome.
```{r, cache = TRUE}
global_summary <- an12_dgg %>% select(-lower, -upper) %>% rename(anthrome = baseline)  %>% anthrome_trajectory()
regional_summary <- an12_dgg %>% select(-lower, -upper) %>% rename(anthrome = baseline)  %>% anthrome_trajectory(by = region_name)
biome_summary <- an12_dgg %>% select(-lower, -upper) %>% rename(anthrome = baseline)  %>% anthrome_trajectory(by = biome)
```
...
ind_summary <- contemp_vars %>%
  ...
  remove_missing() %>%
  # so this join here has two land areas . . . how to handle?
...
# Supplemental Analyses and Visualizations
...
```{r}
change_type %>% count(class_biome, wt =  land_area, name = 'area') %>% 
  mutate(percent = area / sum(area) * 100) %>% write_csv('onsets_biome_area.csv')
```

```{r}
write_csv(change_type, 'change_type.csv')
```
...
```{r, eval = FALSE}
an12_dgg_upper <- read_csv('tmp/an12_dgg_upper.csv', col_types = cols(.default = 'i')) %>%
  dplyr::select(-c(`2001AD`:`2009AD`, `2011AD`:`2016AD`)) %>% 
...
an12_dgg_lower <- read_csv('tmp/an12_dgg_lower.csv', col_types = cols(.default = 'i')) %>%
  dplyr::select(-c(`2001AD`:`2009AD`, `2011AD`:`2016AD`)) %>% 
  left_join(st_drop_geometry(an12_dgg_inputs), by = 'id') %>% 
    left_join(biome_key, by = c('pot_veg' = 'biome_value')) %>% # but join to biome key
  select(-pot_vill, -pot_veg) %>%
...
```{r}
an12_dgg_lower <- an12_dgg %>% select(-baseline, -upper) %>% rename(anthrome = lower)
an12_dgg_upper <- an12_dgg %>% select(-baseline, -lower) %>% rename(anthrome = upper)

```


```{r}
global_summary_upper <- anthrome_trajectory(an12_dgg_upper)
global_summary_lower <- anthrome_trajectory(an12_dgg_lower)
regional_summary_upper <- anthrome_trajectory(an12_dgg_upper, by = region_name)
regional_summary_lower <- anthrome_trajectory(an12_dgg_lower, by = region_name)
biome_summary_upper <- anthrome_trajectory(an12_dgg_upper, by = biome)
biome_summary_lower <- anthrome_trajectory(an12_dgg_lower, by = biome)
```
...
#addSmallLegend(e) + addSmallLegend(f) + plot_layout(guides = 'collect') + plot_annotation(tag_levels = 'a', tag_prefix = '(', tag_suffix = ')') & theme(legend.position = 'bottom')
...
#addSmallLegend(g) + addSmallLegend(h) + plot_layout(guides = 'collect') + plot_annotation(tag_levels = 'a', tag_prefix = '(', tag_suffix = ')') & theme(legend.position = 'bottom')
